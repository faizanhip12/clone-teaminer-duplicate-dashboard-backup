    ...
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        navigator.serviceWorker
          ...
          .then(registration => {
            console.log(registration, 'registration in channel Card')
          })
          .catch(registrationError => {
            console.log(registrationError, 'registrationError in channel Card')
          })
      })
    }
  }, [])
    ...
    <Grid container rowSpacing={1} columnSpacing={{ xs: 0, sm: 2, md: 3 }}>
      {store.status === 'success' && !store.entities.length ? (
        ...
        <Skeleton
          ...
          width={'100%'}
          height={'80vh'}
          sx={{ background: theme.palette.customColors.skeletongrey, mt: 5 }}
        />
      ) : (
