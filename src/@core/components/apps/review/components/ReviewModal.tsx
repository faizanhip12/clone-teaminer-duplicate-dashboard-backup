...
const ModalWrapper = styled('div')<BoxProps>(({ theme }) => ({
  ...
  width: '35%',
  background: theme.palette.linear_gradient.modalGradient,
  textAlign: 'center',
  margin: '5% auto',
  borderRadius: '20px',
  padding: '50px',
...
export const renderClient = (user: IReview) => {
  if (user?.profile_picture) {
    return (
      <CustomAvatar src={user?.profile_picture} sx={{ margin: '0 auto', width: 70, height: 70, borderRadius: '50%' }} />
    )
  } else {
    return (
      <CustomAvatar
        ...
        sx={{ margin: '0 auto', width: 70, height: 70, borderRadius: '50%' }}
      >
