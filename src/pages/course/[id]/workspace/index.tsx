import { Stack, Typography, Box, Button, Skeleton, Grid, TextField, useTheme } from '@mui/material'
      ...
      {ability.can('itsHaveAccess', 'upload-folder-button') ?
        user?.activeChannel?.channel?.id === (store?.entity as IWorkspace)?.course?.channelId && (
          <Box component='div' mt={5}>
            <Button variant='contained' endIcon={<AddIcon />} sx={{ ml: 2 }} onClick={handleDelete}>
              Create New
            </Button>
          </Box>
        ) : null}
