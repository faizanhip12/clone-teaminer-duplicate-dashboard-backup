        ...
        <Grid container spacing={2}>
          {store?.status === 'success' ? (
            !store?.entities?.length ? (
              ...
              store?.entities?.map((item: IPayment, index: number) => {
                return (
                  <Grid item lg={6} md={6} sm={12} xs={12} key={index}>
                    <Item style={{ margin: '2.5rem', background: theme.palette.linear_gradient.cardGradient }}>
                      <Box display={'flex'} justifyContent={'flex-end'}>
                        {ability.can('itsHaveAccess', 'edit-subscription-details') ? <RowOptions id={item?.id} /> : null}
                      </Box>
                      ...
                      <Typography variant='h6' fontSize={'18px'} fontWeight={'600'} lineHeight={'140%'} m={'2.5rem'}>
                        <Box component={'span'} color={theme.palette.customColors.white} fontSize={'72.132px'}>
                          ${item?.price}
                        </Box>
                        /
                        {item?.expireYears >= 1
                      ...
                      <Typography textAlign={'start'} margin={'2.5rem'} fontSize={'12px'}>
                        <Box component={'span'} color={theme.palette.customColors.white}>
                          DISCLAIMER
                        </Box>
            ...
            <Skeleton
              variant='rectangular'
              sx={{ height: '60vh', widows: '75vw', background: theme.palette.customColors.skeletongrey }}
            />
          )}
        </Grid>
      </Box>
