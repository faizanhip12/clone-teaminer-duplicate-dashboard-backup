...
export const fetchAllActionMyCertificates = createAsyncThunk(
  'courses/fetchAllActionMyCertificates',
  async (params: DataParams, { getState, dispatch, rejectWithValue }) => {
    try {
      dispatch(Slice.actions.handleStatus('pending'))
      const { query } = params
      const response = await coursesService.getAllMyCertificates({ query })
      dispatch(Slice.actions.handleStatus('success'))
      return response.data
    } catch (error) {
      return ApiError(error, dispatch, rejectWithValue)
...
export const Slice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(fetchAllActionMyCertificates.fulfilled, (state, action) => {
      const { data } = action.payload
      state.entities = data?.certificates || []
    })
