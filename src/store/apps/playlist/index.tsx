...
export const fetchAllActionOfEveryCourse = createAsyncThunk(
  'playlist/fetchAllActionOfEveryCourse',
  async ({ query }: GetParams, { dispatch, rejectWithValue }) => {
    try {
      dispatch(Slice.actions.handleStatus('pending'))
      const response = await playlistService.getAllCatalogueCourses({ query })
      dispatch(Slice.actions.handleStatus('success'))
      return response.data
    } catch (error: any) {
      return ApiError(error, dispatch, rejectWithValue)
...
export const Slice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(fetchAllActionOfEveryCourse.fulfilled, (state, action) => {
      const { data } = action.payload
      state.entities = data.entities || []
      // state.total = data.entities?.length || 0
    })
