...
export const fetchPoints = createAsyncThunk("students/pointsWeekly", (query: any) => query)
...
export const Slice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAllActionPoints.fulfilled, (state, action) => {
      const { data } = action.payload;
      state.allPoints = data.points
      state.entities = data?.points.all;
    });
    builder.addCase(fetchPoints.fulfilled, (state, action) => {
      const allEntities = { ...state.allPoints }
      if (action.payload === "Weekly") {
        state.entities = allEntities.weekly
      } else if (action.payload === "Monthly") {
        state.entities = allEntities.monthly
      } else if (action.payload === "Daily") {
        state.entities = allEntities.daily
      } else {
        state.entities = allEntities.all
      }
    });
